...
MySQL.prototype.AWSSettings = function (region, dbPort, dbUsername, dbName, dbEndpoint,accessKeyId,secretAccessKey) {
  MySQL.prototype.region = region;
  MySQL.prototype.dbPort = dbPort;
  MySQL.prototype.dbUsername = dbUsername;
  MySQL.prototype.dbName = dbName;
  MySQL.prototype.dbEndpoint = dbEndpoint;
  MySQL.prototype.accessKeyId = accessKeyId;
  MySQL.prototype.secretAccessKey = secretAccessKey;
}
...
MySQL.prototype.connect = function (callback) {
  ...
  if (this.client) {
    if (callback) {
    ...
    } else if (MySQL.region) {
      const region = MySQL.region;
      const dbPort = MySQL.dbPort;
      const dbUsername = MySQL.dbUsername; // the name of the database user you created in step 2
      const dbName = MySQL.dbName; // the name of the database your database user is granted access to
      const dbEndpoint = MySQL.dbEndpoint;
      var creds = new AWS.Credentials({
        accessKeyId: MySQL.accessKeyId, secretAccessKey: MySQL.secretAccessKey
      });
      ...
      signer.getAuthToken({ // uses the IAM role access keys to create an authentication token
        ...
        if (err) {
          ...
          var mysqlOptions = AWS.util.merge(options, dbOptions);
          // console.log('mysqlOptions:',mysqlOptions);
          // console.log("cliente",this.client);
          this.client = mysql.createConnection(mysqlOptions);
      ...
      console.error('RDS settings failed');
      callback(new Error("RDS settings not detected."));
    }
  }
};
...
function generateOptions(settings) {
  ...
  if (s.url) {
    ...
    options = {
      host: s.host || s.hostname || 'test-database.proxy-ccsibpezv4qx.us-east-1.rds.amazonaws.com',
      port: s.port || 3306,
